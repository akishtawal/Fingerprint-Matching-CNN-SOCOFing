...
sample_fraction = 0.1  # Use 20% of the data
...
x1 = layers.Input(shape=(90, 90))
x2 = layers.Input(shape=(90, 90))
...
# precision_value = model.evaluate(x_val, y_true, verbose=0)[2]
# recall_value = model.evaluate(x_val, y_true, verbose=0)[3]
# accuracy_value = model.evaluate(x_val, y_true, verbose=0)[4]

# precision_value = model.evaluate([x_val[:, :, :, np.newaxis]], y_true, verbose=0)[2]
# recall_value = model.evaluate([x_val[:, :, :, np.newaxis]], y_true, verbose=0)[3]
# accuracy_value = model.evaluate([x_val[:, :, :, np.newaxis]], y_true, verbose=0)[4]
...
# print(y_true[:5])  # Print the first 5 samples of y_true

# def custom_evaluate(model, x1, x2, y_true):
#     batch_size = 32
...
#         x1_batch = x1[start:end, :, :, np.newaxis]
#         x2_batch = x2[start:end, :, :, np.newaxis]
#         y_true_batch = y_true[start:end, :, 0, 0]  # Get the first element of the last two dimensions

#         y_pred_batch = model.predict([x1_batch, x2_batch])
#         loss, precision, recall, accuracy = model.test_on_batch([x1_batch, x2_batch], y_true_batch)

#         total_loss += loss * len(x1_batch)
#         total_precision += precision * len(x1_batch)
#         total_recall += recall * len(x1_batch)
#         total_accuracy += accuracy * len(x1_batch)

#     total_loss /= len(x1)
#     total_precision /= len(x1)
#     total_recall /= len(x1)
#     total_accuracy /= len(x1)

#     return total_loss, total_precision, total_recall, total_accuracy

# Extract the binary labels from the one-hot encoded representation
y_true_binary = np.argmax(y_true, axis=-1).astype(float)
y_true_ohe = to_categorical(y_true_binary)

y_pred_binary = np.round(y_pred).squeeze()

# Evaluate the model using the custom evaluation function
# loss, precision_value, recall_value, accuracy_value = custom_evaluate(model, x_val, x_val, y_true)
loss, precision, recall, accuracy = model.evaluate([x_val[:, :, :, np.newaxis], x_val[:, :, :, np.newaxis]], y_true_ohe)
...
model.save('SOCOFing_Fingerprint_Matching_V2.keras')